...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyDown(KEY_W)) W_ALL  -= 1.0f * dt;
        if (IsKeyDown(KEY_S)) W_ALL  += 1.0f * dt;
        if (IsKeyDown(KEY_E)) W_COES -= 1.0f * dt;
        if (IsKeyDown(KEY_D)) W_COES += 1.0f * dt;
        if (IsKeyPressed(KEY_R)) {
            W_SEP = 1.5f; W_ALL = 1.0f; W_COES = 1.0f;
            inizializza_stormo(stormo);
        }
        if (W_SEP  < 0) W_SEP  = 0;
        if (W_ALL  < 0) W_ALL  = 0;
        if (W_COES < 0) W_COES = 0;
        ...
        DrawText(TextFormat("W_ALL  = %.2f  (W-/S+)", W_ALL),  10, 32, 18, WHITE);
        DrawText(TextFormat("W_COES = %.2f  (E-/D+)", W_COES), 10, 54, 18, WHITE);
        DrawText("R = reset", 10, 80, 16, GRAY);
        EndDrawing();
    }
