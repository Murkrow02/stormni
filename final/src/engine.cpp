...
void Engine::draw() {
    ...
    // Draw boundaries
    DrawCubeWires({0, 0, 0}, 2 * BOX_HALF_X, 2 * BOX_HALF_Y, 2 * BOX_HALF_Z, DARKGRAY);
    ...
    for (const auto& boid : boids) {
        ...
        DrawCylinderEx(to_rl(boid->get_pos()), topPos, boid->get_cone_base_r(), 0.0f, 12,
                       boid->get_color());
    }
...
void Engine::draw_stats() {
    ...
    auto panel = [&](const char* name, const char* breed, int px, int py) {
        ...
        int tx = px + 8, ty = py + 5;
        DrawText(TextFormat("%s (%d)", name, s.n), tx, ty, 18, head_txt);
        ty += hh + 4;
        DrawText(TextFormat("Avg speed %.1f", s.avg), tx, ty, 14, RAYWHITE);
        ty += 18;
        DrawText(TextFormat("Max speed %.1f", s.max), tx, ty, 14, RAYWHITE);
        ty += 18;
