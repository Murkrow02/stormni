// Soluzione step 11 — murmuration 3D completa.
#include "raylib.h"
#include "vec3.h"
#include <vector>
#include <cstdlib>
...
static Vec3 applica_wrap(Vec3 p) {
...
int main() {
    ...
    for (int i = 0; i < N; ++i)
        stormo.push_back(Boid{
            { casuale(-BOX, BOX), casuale(-BOX, BOX), casuale(-BOX, BOX) },
            { casuale(-100, 100), casuale(-100, 100), casuale(-100, 100) }
        });
