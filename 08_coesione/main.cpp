...
//     }
//     if (k == 0) return Vec2{0, 0};
//     Vec2 p_media = mul(somma, 1.0f / k);
//     return sub(p_media, s[i].pos);
// }
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            // TODO 3a: coes[i] = calcola_coesione(stormo, i);
        }
        for (int i = 0; i < N; ++i) {
            Vec2 acc = add(mul(sep[i], W_SEP), mul(all_[i], W_ALL));
            // TODO 3b: somma il contributo di coesione.
            // acc = add(acc, mul(coes[i], W_COES));
